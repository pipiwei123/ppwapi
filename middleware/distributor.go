...
func Distribute() func(c *gin.Context) {
	return func(c *gin.Context) {
		...
		// 检查是否为多分组模式
		isMultiGroup, _ := common.GetContextKey(c, "token_is_multi_group")

		if tokenGroup != "" && isMultiGroup != true {
			// 单分组模式：检查分组可用性
		...
		if ok {
			...
			if shouldSelectChannel {
				...
				// 获取多分组信息
				allGroups, _ := common.GetContextKey(c, "token_all_groups")

				if isMultiGroup == true && allGroups != nil {
					// 多分组模式：依次尝试每个分组
					groupList := allGroups.([]string)
					var lastErr error

					for _, tryGroup := range groupList {
						// 检查分组可用性
						if _, ok := setting.GetUserUsableGroups(userGroup)[tryGroup]; !ok {
							// 分组不可用，跳过
							continue
						}

						// 检查分组是否在比率配置中
						if !ratio_setting.ContainsGroupRatio(tryGroup) {
							if tryGroup != "auto" {
								// 分组已被弃用，跳过
								continue
							}
						...
						channel, selectGroup, err = model.CacheGetRandomSatisfiedChannel(c, tryGroup, modelRequest.Model, 0)
					...
					// 如果所有分组都失败了
					if channel == nil {
						message := "令牌的所有分组都无可用渠道"
						if lastErr != nil {
							message = fmt.Sprintf("令牌的所有分组下对于模型 %s 都无可用渠道", modelRequest.Model)
						}
...
func getModelRequest(c *gin.Context) (*ModelRequest, bool, error) {
	...
	if strings.HasPrefix(c.Request.URL.Path, "/pg/chat/completions") {
		...
		// 对于Playground模式，如果分组为空或者是数字ID，使用用户分组
		if modelRequest.Group == "" || isNumericString(modelRequest.Group) {
			userGroup := common.GetContextKeyString(c, "group")
