...
export const usePlaygroundState = () => {
  ...
  const [inputs, setInputs] = useState(() => {
    const configInputs = savedConfig.inputs || DEFAULT_CONFIG.inputs;
    // 修复错误的group值（如果是数字ID，清空它）
    if (configInputs.group && /^\d+$/.test(configInputs.group)) {
      console.info('检测到localStorage中保存的无效分组值（数字ID）:', configInputs.group, '已重置为空字符串');
      configInputs.group = '';
    }
    return configInputs;
  });
