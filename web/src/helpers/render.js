  ...
  return (t) => {
    ...
    categoriesCache = {
      ...
      seedance: {
        ...
        icon: <OpenAI />,
        filter: (model) =>
          model.model_name.toLowerCase().includes('seedance') ||
          model.model_name.toLowerCase().includes('seedream'),
      },
      sora: {
        label: 'Sora',
        icon: <OpenAI />,
...
export const renderGroupOption = (item) => {
  ...
  const getPriorityIcon = (priority) => {
    if (priority <= 1) return '●'; // 实心圆表示最高优先级
    if (priority <= 5) return '◆'; // 菱形表示高优先级
    if (priority <= 10) return '▲'; // 三角形表示中等优先级
    return '◇'; // 空心菱形表示低优先级
  };

  // 根据优先级获取颜色
  const getPriorityColor = (priority) => {
    if (priority <= 1) return '#FFD700'; // 金色
    if (priority <= 5) return '#00BFFF'; // 深蓝色
    if (priority <= 10) return '#32CD32'; // 绿色
    return '#808080'; // 灰色
  };
