  ...
  const loadGroups = async () => {
    ...
    if (success) {
      ...
      if (statusState?.status?.default_use_auto_group) {
        const hasAutoGroup = localGroupOptions.some((group) => group.value === 'auto');
        if (!hasAutoGroup) {
                    ...
                    <Form.Switch
                      ...
                      extraText={
                        <div className='text-sm text-gray-600 mt-1'>
                          <div className='mb-1'>{t('启用后可为令牌配置多个分组，支持按优先级自动切换')}</div>
                          <div className='text-xs text-blue-600'>
                            {t('示例：优先使用VIP分组（折扣价），不可用时自动切换到默认分组')}
                          </div>
                        </div>
                      }
                    />
                            ...
                            extraText={
                              <div className='text-sm space-y-1'>
                                <div className='text-gray-600'>
                                  {t('分组将按选择顺序调用，第一个分组不可用时自动切换到下一个')}
                                </div>
                                <div className='text-blue-600 text-xs'>
                                  {t('建议：将折扣分组放在前面，可以节省成本')}
                                </div>
                                <div className='text-red-600 text-xs'>
                                  {t('注意：多分组模式不支持auto分组，如需使用auto分组请切换为单分组模式')}
                                </div>
