    ...
    <Card>
      <div style={{ marginBottom: 16 }}>
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <Tooltip
              content={t(
                '配置特定用户分组使用特定令牌分组时的特殊倍率，未配置则使用默认倍率'
              )}
            >
        ...
        <Text type="secondary" size="small">
          {t(
            '配置不同用户分组使用不同令牌分组时的特殊价格倍率，空白表示使用默认倍率'
          )}
        </Text>
      </div>
      ...
      >
        ...
        >
          <div style={{ marginBottom: 16 }}>
            <Text type="secondary" size="small">
              {t('配置用户分组')} <Tag color="cyan">{editingUserGroup}</Tag>{' '}
              {t('使用不同令牌分组时的特殊倍率，留空表示使用默认倍率')}
            </Text>
          </div>
          ...
          {tokenGroups.map((tokenGroup) => (
            <Form.InputNumber
              ...
              suffix={
                <Tooltip
                  content={`${t('当前值')}: ${getRatioText(getRatio(editingUserGroup, tokenGroup))}`}
                >
                  <Tag
                    color={getRatioColor(
                      getRatio(editingUserGroup, tokenGroup)
                    )}
                    size="small"
                  >
                    {getRatio(editingUserGroup, tokenGroup)
          ...
          >
            <Text type="secondary" size="small">
              <strong>{t('说明')}:</strong>
              <br />• {t('设置该用户分组使用不同令牌分组时的价格倍率')}
              <br />• {t('留空的令牌分组将使用该分组的默认倍率')}
              <br />• {t('此配置优先级高于基础分组倍率配置')}
            </Text>
          </div>
        </Form>
      </Modal>
