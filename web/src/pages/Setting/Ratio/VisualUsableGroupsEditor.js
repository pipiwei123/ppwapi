...
import { IconPlus, IconDelete, IconEdit, IconSave, IconClose, IconInfoCircle } from '@douyinfe/semi-icons';
  ...
  const columns = [
        ...
        if (isEditing(record)) {
          return (
            <Form.Input
              ...
              style={{ margin: 0 }}
              rules={[
                { required: true, message: t('分组描述不能为空') }
              ]}
            />
          );
        }
    ...
    },
    {
      title: (
        <Space>
          {t('优先级')}
          <Tooltip content={t('数字越小优先级越高，影响分组在下拉列表中的排序')}>
  ...
  return (
    <Card>
      <div style={{ marginBottom: 16 }}>
        ...
        <Text type='secondary' size='small'>
          {t('配置用户创建令牌时可以选择的分组，优先级影响分组在下拉列表中的排序')}
        </Text>
      </div>
      ...
      >
        <Table
          dataSource={data.sort((a, b) => (a.priority || 999) - (b.priority || 999))}
          columns={columns}
          pagination={false}
          size='small'
          empty={
